...
sp_name       = lambda fmt, *args, **kwargs: SP_TEST_PREFIX + fmt.format(*args, **kwargs)
sp_name_check = lambda name: name.startswith(SP_TEST_PREFIX)
...
reassignHandler = [
	...
	lambda data: api.volumesReassignWait({'reassign': data, 'attachTimeout': 1}),
	lambda data: api.volumesReassignWait({'reassign': data, 'attachTimeout': 0}),
]
...
def showState():
	...
	print "PLACEMENT GROUPS:", [pgName for pgName in api.placementGroupsList()]
...
def setup(no_ssd=False):
	...
	print api.placementGroupUpdate(sp_name("hdd"), { 'addDisks': [disk.id for disk in disks if not disk.ssd] })
	if no_ssd:
		print '- placementGroup create ssd with all the disks'
		print api.placementGroupUpdate(sp_name("ssd"), { 'addDisks': [disk.id for disk in disks] })
	else:
		print '- placementGroup create ssd'
		print api.placementGroupUpdate(sp_name("ssd"), { 'addDisks': [disk.id for disk in disks if disk.ssd] })
	
	print '- template create 1'
	print api.volumeTemplateCreate({ 'name': sp_name("tmpl_1"), 'size': 10 * 1024 ** 3, 'replication': 1, 'placeAll': sp_name("hdd"), 'placeTail': sp_name("ssd") })
	print '- template create 2'
	print api.volumeTemplateCreate({ 'name': sp_name("tmpl_2"), 'size': 10 * 1024 ** 3, 'replication': 2, 'placeAll': sp_name("hdd"), 'placeTail': sp_name("ssd") })
	print '- template create 3'
	print api.volumeTemplateCreate({ 'name': sp_name("tmpl_3"), 'size': 10 * 1024 ** 3, 'replication': 3, 'placeAll': sp_name("hdd"), 'placeTail': sp_name("ssd") })
	
	for v in xrange(5):
		for r in xrange(1, 4):
			print '- volume {v} template {r}'.format(v=v, r=r)
			volName = sp_name("volume_{v}_{r}", v=v, r=r)
			print api.volumeCreate({ 'name': volName, 'template': sp_name("tmpl_{r}", r=r) })
	...
	for vol in api.volumesList():
		...
		print api.volumeCreate({ 'name': vol.name + "_clone", 'baseOn': vol.name })
...
def scrubbing():
	diskId = api.disksList().values()[0].id
	...
	assert api.disksList()[diskId].scrubbingPaused == False
...
def remote():
	...
	while True:
		r = [r for r in api.snapshotsRemoteList()['snapshots'] if r.name == vol.name]
		if r:
			break;
	...
	rem = r[0]
	api.snapshotFromRemote({ 'name':"restoredSnapshot", 'remoteId' : rem.remoteId, 'remoteLocation' : loc.name, 'template' : "sptest_tmpl_1"})
	api.snapshotExport({'snapshot':"restoredSnapshot", 'location':loc.name})
	api.snapshotUnexport({'snapshot':"restoredSnapshot", 'location':loc.name})
...
def main():
	parser = argparse.ArgumentParser(
		...
		usage='''
	sptest [--no-ssd] [phase...]''')
	parser.add_argument('--no-ssd', action='store_true', help='run on HDDs only')
	parser.add_argument('phases', nargs='*')
	
	args = parser.parse_args()
	if not args.phases:
		args.phases = ['cleanup', 'setup', 'config', 'volumes', 'relocator', 'cleanup', 'scrubbing', 'remote']
	phases = []
